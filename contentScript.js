...
new MutationObserver(() => {
  ...
  if (url !== lastUrl) {
    ...
    // Reset the skipReflectionPopup flag when navigating to a new URL
    chrome.storage.local.set({ 'skipReflectionPopup': false });
    // Re-detect shopping intent and product page
    detectShoppingIntent();
    detectProductPage();
  }
...
function detectShoppingIntent() {
  ...
  // Check if we should skip showing the popup
  chrome.storage.local.get(['skipReflectionPopup'], function(data) {
    if (data.skipReflectionPopup) {
      console.log("Skipping reflection popup due to user choice");
      return;
    }
...
function showReflectionPopup(e) {
  ...
  chrome.storage.local.get(['currentProduct'], function(data) {
    ...
    document.getElementById('dineed-yes').addEventListener('click', function() {
      ...
      // Store a flag in local storage to prevent the popup from showing again for this product
      chrome.storage.local.set({ 'skipReflectionPopup': true }, function() {
        console.log("Set skipReflectionPopup to true");
        ...
        if (e && e.target) {
          setTimeout(() => {
            try {
              console.log("Attempting to trigger original click");
              
              // Create a new MouseEvent without our interceptor
              ...
              // Temporarily remove our dataset flag
              if (originalButton.dataset) {
                delete originalButton.dataset.dINeedListenerAttached;
              }
              
              // Find the closest form if it exists
              const form = originalButton.closest('form');
              if (form) {
                console.log("Found form, submitting directly");
                form.submit();
              } else {
                // If no form, dispatch the event directly on the button
                console.log("No form found, dispatching click event");
                originalButton.dispatchEvent(clickEvent);
              }
              
              // Restore the dataset flag after a longer delay to ensure the click has been processed
              setTimeout(() => {
                if (originalButton && originalButton.dataset) {
                  console.log("Restoring dataset flag");
                  originalButton.dataset.dINeedListenerAttached = "true";
                }
              }, 2000);
            } catch (clickError) {
              console.error("Error triggering original click:", clickError);
            }
          }, 500);
        }
      });
...
function showSavedMessage() {
  // Set the flag to prevent the popup from showing again for this product
  chrome.storage.local.set({ 'skipReflectionPopup': true });
...
function detectProductPage() {
  ...
  // Reset the skipReflectionPopup flag when navigating to a new product page
  chrome.storage.local.set({ 'skipReflectionPopup': false });
